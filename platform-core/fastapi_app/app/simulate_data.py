...
def send_request(url, payload):
    ...
    try:
        ...
        print(f"Successfully sent to {url.split('/')[-2].replace('-',' ')}: {payload.get('transaction_id') or payload.get('claim_id')}. Response: {response.json()}")
...
if __name__ == "__main__":
    print(f"Starting data simulation. Sending {NUM_REQUESTS_PER_BATCH} requests every {DELAY_SECONDS} seconds...")
    print(f"Target FastAPI: {FASTAPI_URL}")

    while True:
        try:
            for _ in range(NUM_REQUESTS_PER_BATCH // 2): # Send half financial, half insurance
                # Send financial transaction
                financial_data = generate_financial_transaction()
                send_request(FINANCIAL_ENDPOINT, financial_data)

                # Send insurance claim
                insurance_data = generate_insurance_claim()
                send_request(INSURANCE_ENDPOINT, insurance_data)
